...
BASE = {
    ...
    "issuers": {"BBT": {"name": "Bybit"}, "RVL": {"name": "Client B"}, "NXP": {"name": "Client C"}},
}
...
def test_bbt_auth_returns_action_code_000():
    cfg = {**BASE, "active_relay_path": "secondary"}
    assert route_auth("BBT", cfg)["action_code"] == "000"


def test_no_regression_other_issuers():
    cfg = {**BASE, "active_relay_path": "secondary"}
    for issuer in ("RVL", "NXP"):

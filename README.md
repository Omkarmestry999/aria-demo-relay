...
This is a **dummy repository we own** — NOT Thredd's code. It exists so the Remediation
stage of UC-01 can be shown for real (a genuine PR + a real CI run) without needing the
client's actual codebase. The patterns mirror our incident (Hyperion issuer-relay failover).

## What's here
| Path | Role in the demo |
|---|---|
| `config/hyperion/relay-routing.yaml` | The file the agent's fix edits (`active_relay_path: primary -> secondary`). |
